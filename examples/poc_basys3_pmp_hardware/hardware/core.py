...
CPU_VARIANTS = {
    ...
    "secure+poc":         "VexRiscv_Secure_PoC"
}
...
GCC_FLAGS = {
    ...
    "secure+poc":       "-march=rv32i2p0_ma    -mabi=ilp32"
}
...
class VexRiscv(CPU, AutoCSR):
    ...
    def __init__(self, platform, variant="standard", with_timer=False):
        ...
        self.pmp_input        = Signal()
        self.pmp_output       = Signal()
        # # #
        ...
        if "poc" in variant:
            self.cpu_params.update(
                o_o_pmphit              = self.pmp_output,
                i_i_pmphit              = self.pmp_input
            )
